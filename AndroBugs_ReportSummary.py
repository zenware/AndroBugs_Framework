...
def parseArgument():
    parser = argparse.ArgumentParser(
        description='AndroBugs Framework: Android APK Vulnerability Summary Reporter (Parameter Condition: and)')
    ...
    parser.add_argument("-s", "--analyze_status", help="\"success\", \"fail\", or \"all\"(default)", type=str,
...
print("%50s   %9s %9s %9s %9s %9s %17s %14s %14s %14s %17s" % ("Vector Name",
                                                               ...
                                                               "% of Critical", "% of Warning", "% of Notice",
                                                               "% of Info", "% of Non-Info"))
print('-' * 190)
    ...
    for key, level_count_list in sorted(vector_to_level_count_list.items()):
        if count_success == 0:
            print("%50s : %9s %9s %9s %9s %9s %16.2f%% %13.2f%% %13.2f%% %13.2f%% %16.2f%%" % (
                key,
                level_count_list["Critical"], level_count_list["Warning"], level_count_list["Notice"],
                level_count_list["Info"],
                count_success,
                0, 0, 0, 0, 0
            ))
        else:
            print("%50s : %9s %9s %9s %9s %9s %16.2f%% %13.2f%% %13.2f%% %13.2f%% %16.2f%%" % (
                ...
                level_count_list["Info"],
                count_success,
                (level_count_list["Critical"] / float(count_success) * 100),
                (level_count_list["Warning"] / float(count_success) * 100),
                (level_count_list["Notice"] / float(count_success) * 100),
                (level_count_list["Info"] / float(count_success) * 100),
...
if (count_total > 0):
    print(
        "Total(Success + Fail to analyze) APKs: %d;  Success to analyze APKs: %d (%.2f%%);  Fail to analyze APKs: %d (%.2f%%)" % (
            count_total, count_success, ((count_success / float(count_total)) * 100), count_fail,
            ((count_fail / float(count_total)) * 100)
        ))
